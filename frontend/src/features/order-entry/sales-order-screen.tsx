...
import { Boxes, FileText, XCircle } from "lucide-react";
...
export function SalesOrderScreen({ orderId }: { orderId: number }) {
  ...
  return (
    <DocumentWorkspaceShell
      backHref="/oe/sales-orders"
      title={data.number}
      subtitle={dotted(partnerCode(partner!, "ar") ?? "", partner?.name ?? "")}
      statusChip={
          ...
          <div className="flex items-center gap-3">
            ...
            {isOpen && canManage ? (
              <Link
                ...
                className="inline-flex h-10 items-center justify-center gap-2 rounded-[var(--radius-control)] bg-[var(--vinea-brand)] px-4 text-sm font-medium text-[var(--vinea-on-brand)] hover:bg-[var(--vinea-brand-strong)]"
