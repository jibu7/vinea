...
10. **Migrations apply from zero.** Never edit an applied migration; never add "emergency" fix scripts. `alembic upgrade head` on an empty DB must always succeed (CI gate). **A migration that UPDATEs or DELETEs on a posted table is tested against posted rows** — provision a tenant at the prior revision *with the rows the migration is meant to touch*, then upgrade, the way `tests/test_p5_backfill.py` does. `make migrate-check` runs on an empty scratch database and therefore proves DDL and nothing else: a P5 step 9 back-fill of `journal_entries.source_doc_id` passed it cleanly and could never have run on a real tenant, because `kernel_block_posted_entry_mutation` raises `VN001` on any UPDATE to a posted entry and `inventory_block_move_mutation` does the same for every move. Both are unconditional and have no escape hatch, which is the point — if a back-fill needs `DISABLE TRIGGER`, the answer is that the data cannot be back-filled, not that the trigger is in the way (see rule 3). Fix forward and resolve history from the side that still works.
...
14. **Every mutating endpoint has a caller, or an exemption with a reason.** `backend/tests/test_api_has_a_caller.py` enumerates every POST/PUT/PATCH/DELETE the app serves and fails on any with no call site under `frontend/src` and no entry in its `NO_UI` list. A service that ships as an endpoint with no caller is a capability the product does not have: it passes every test, appears in the OpenAPI schema, and nobody can use it. This has cost the build three times — P4's `mature_instruments` (Appendix C.1.6), P5's `useReverseStockDocument` (C.1.7), and the AR/AP document reversal and unallocation, which were still unreachable when P5 closed and were found by reading code rather than by a test. Exemptions say which kind they are: `by design` for something no person should press (a retention reaper), `GAP (<phase>)` for debt, so the list reads as a register and every addition costs a line of review.

## Quality bar (every task)
- `make be-lint && make be-test` (ruff, then pytest) must be green before you report done. Both run **inside the backend container**: the container writes `backend/.venv` as root through the bind mount, so a host `uv run` dies on `.venv/CACHEDIR.TAG` on any machine that has brought the stack up. Add tests with the code, including accounting-invariant assertions where money moves.
