...
| P8 | 3 | Payment runs and supplier bank details | `app/banking/payment_runs.py` and `remittance.py`, the nine payment-run endpoints with their rule-14 lines, `payment_run_member` on P4's `reverse_document`, supplier bank details through `PartnerInput`, `assert_bank_invariants` clause 8 with four sensitivity tests, the machine's three new operations and the run-reversal targeted property, and `docs/p8-step-3-report.md` | `86d65f2` / PR #66 | "Step 3 approved; arm auto-merge on #66. Decisions: (1) the P4 defect is `allocate()` accepting an allocation dated before one of its documents — fix it as its own PR off `main` now (`allocation_before_document`, both allocate paths, the reduced P4 reproduction as the sensitive test, `fix(subledger): an allocation is dated no earlier than the documents it joins`); `payment_run_before_invoice` stays as a separate rule. (2) No idempotency column for reverse/reopen — state refusals suffice; record as a deviation. (3) Discount on full settlement only — record as the decision-7 reading." — Owner, 2026-09-22. **The first decision corrected the build's diagnosis, not just its scope.** The report proposed guarding `allocate()` against an allocation dated before the *latest* document; the owner's reading is the sharper one — a payment dated before its invoice is a **deposit** and ordinary business, so what must stay legal is the payment and what must be refused is *allocating* it on a date the invoice was not yet posted. Both guards are kept because neither implies the other. |
