...
| P5 | 5 | Enquiry, reports and the costing tape | item enquiry, the Movement / Transaction / Valuation / Count reports, and the document-level costing tape (`backend/tests/inventory/test_costing_tape_documents.py`) | *pending — the commit this branch pushes* | **Awaiting the owner.** Opened at submission, not filled in on the author's behalf. Replace this cell with the owner's words and pin the commit they were looking at. The step-5 report carries the tape output table, the invariant-suite output and the sensitivity pass; nothing proceeds to step 6 (UI) until this row has words in it. |
