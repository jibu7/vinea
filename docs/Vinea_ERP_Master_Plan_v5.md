...
6. **Nothing matures a post-dated instrument** — decision 7 books a receipt or payment dated ahead to the post-dated receivable/payable account instead of bank, and `mature_instruments(as_of)` moves it across on or after maturity. The plan exposed that service "as an endpoint and a scheduled job", which left the product able to *raise* an instrument and unable to *bank* one: no screen listed what was outstanding, and the only way to discover a cheque existed was to run maturity and see whether anything moved. The nav adds **"Post-dated receipts"** under Transactions → AR and **"Post-dated payments"** under Transactions → AP (P4 step 9), after Allocate. Each lists every instrument whose cash has not landed — due or not — and banks the ones matured by a chosen date, in one run through the PostingEngine. The run is per-date rather than per-row because the service takes a date and posts the transfers in one transaction; a per-row button would either misrepresent that or need a second, narrower service behind it. Due-date *dashboard widgets* remain P9/P10 (§Appendix A) — this is the transaction screen, not the alerting.

7. **Nothing reverses a stock document** — P5 decision 11 gives every one of them a reversal (kernel reversal plus reversing moves at the original values), and the phase shipped that as an endpoint with no screen: `useReverseStockDocument` sat in the frontend hooks with no caller. The only Reverse button in the product was the general ledger's, on `/gl/entries/{id}` — which is where an adjustment lands after posting, and which posts the reversing *entry* and no reversing moves. On an inventory adjustment it therefore moved the inventory account and left every stock move where it was: `assert_stock_invariants` fails with "INV line N has no stock move behind it", the phase invariant undone from a button. AR and AP had the identical hole from P4, for the identical reason.

   The kernel now refuses it — `posting.module_reversal(module)` is a window the owning module's own service opens, and a reversal asked for anywhere else raises `reverse_via_module_document` — which would have left a document the product could post and never undo. So the nav adds **"Documents"** under Transactions → Inventory (P5 step 9): `/inventory/documents` lists adjustments, journal batches and count-variance documents with their kind, warehouse, type, value and status, and `/inventory/documents/{id}` carries the Reverse action under `inv:transactions_adjust`.
...
11. **Transactions → Tax is four screens, and the owner's tree has no Tax block under
   Transactions at all** — it carries Tax under Maintenance only, which was right for what was
   there. These four are not maintenance. A queue row is a declaration in flight; a feed row is
   a purchase somebody has to say yes or no to; an import declaration is an acknowledgment the
   authority is waiting for; and a VAT return posts a settlement entry through the kernel. Every
   one is a transaction: done on a day, by a person, with a consequence in the ledger or at the
   authority. The nav adds, in this order (P7 step 7):

   | row | route | what it does |
   |---|---|---|
   ...
   on the buttons. The VAT return reads on `tax:vat_return_view` or `tax:vat_return_file`, and
   File checks the second on the button: an accountant who may read the return and not submit
   it is a real arrangement.

   The rest of what P7 step 7 built is again **sections on screens the tree already carries**
...
12. **Transactions → GL gains FX revaluation** — `/gl/fx-revaluations`, after Cashbook batches
   ...
   Cashbook batches. The *report* over a posted run is step 8's, under Reports → General
   Ledger. Its own permission, `gl:fx_revalue`: revaluing is a month-end act an accountant
   performs, not something everyone who reads the ledger should be able to press.
